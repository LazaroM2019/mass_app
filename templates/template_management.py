...
def load_dynamic_template(name: str, title=None, message=None, media_id=None):
    if "general_text_dynamic" == name:
        template_path = "templates/schema/general_text_dynamic.json"
    if "general_image_dynamic" == name:
        template_path = "templates/schema/general_image_dynamic.json"
    if "general_doc_dynamic" == name:
        template_path = "templates/schema/general_doc_dynamic.json"    
...
def load_prompt_template(name:str):
    ...
    if "message_summary" == name:
        template_path = "templates/schema_prompts/message_summary.json"
    with open(template_path, "r") as file:
        template_schema = json.load(file)
    return template_schema
